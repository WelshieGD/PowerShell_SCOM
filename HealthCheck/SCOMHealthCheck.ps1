  ...
  $AvailableAgents = get-scomclass -name 'microsoft.systemcenter.agent' | Get-SCOMClassInstance | Select DisplayName, IsAvailable, AvailabilityLastModified
...
"@
  ...
  ##################################################################
  # Show all Agents that are in an Unavailable State (Greyed out)
  Write-Host "Checking for Unavailable Agents"

  $ReportOutput += "<br>"
  $ReportOutput += "<h2>Unavailable Agents</h2>"
  If (!($AvailableAgents | Where-Object -FilterScript {$_.IsAvailable -eq $False} | Select-Object -Property DisplayName, IsAvailable, AvailabilityLastModified))
  {
    $ReportOutput += "<p>No Agents are in the Unavailable State</p>"
  }
  Else
  {
    
    $ReportOutput += $AvailableAgents | Where-Object -FilterScript {$_.IsAvailable -eq $False} | Select-Object -Property DisplayName| ConvertTo-HTML -fragment
  }
  ...
  $ReportOutput += "<p>The above figures include agents who may be unavailable - their health state will be listed as the health state when they became unavailable</p>"
  $ReportOutput += "<p>Please see the section above for agents which are greyed out (listed as unavailable)</p>"
...
where Name is NULL and IsInMaintenanceMode = 1
"@
